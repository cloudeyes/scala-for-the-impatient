...
object Unidoc {
  ...
  val (unidocSettings, javadocSettings) =
    if (Config.genjavadocEnabled) (scalaJavaUnidocSettings, genjavadocSettings)
    else (scalaUnidocSettings, Nil)

  def scaladocSettings: Seq[Setting[_]] = {
    scaladocSettingsNoVerificationOfDiagrams ++
      (if (Config.scaladocDiagramsEnabled) Seq(doc in Compile ~= scaladocVerifier) else Seq.empty)
  }
  ...
  def scaladocSettingsNoVerificationOfDiagrams: Seq[Setting[_]] = {
    inTask(doc)(Seq(
      ...
      autoAPIMappings := Config.scaladocAutoAPI
    ))
  }

  def scaladocOptions(ver: String, base: File): List[String] = {
    val urlString = Config.scmInfo.map(_.browseUrl + "/blob/master€{FILE_PATH}.scala").getOrElse("")
    val opts = List("-implicits", "-doc-source-url", urlString, "-sourcepath", base.getAbsolutePath)
    if (Config.scaladocDiagramsEnabled) "-diagrams"::opts else opts
  }
